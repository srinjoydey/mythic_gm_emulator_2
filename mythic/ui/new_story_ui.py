...
class CharactersThreadsTablesUI(QWidget):
    ...
    def __init__(self, parent, controller):
        ...
        # Configure grid layout dynamically
        self.layout = QGridLayout(self)
        self.layout.setSpacing(0)  # No spacing between cells for a clean table look
        ...
        for section_index, section_label in enumerate(section_labels):
            ...
            section_label_widget = QLabel(section_label, self)
            ...
            section_label_widget.setStyleSheet("""
                                               ...
                                               font-weight: bold;
                                               """)
            self.layout.addWidget(section_label_widget, row_index, 0, 5, 2)  # Spanning 5 rows, 2 columns
            ...
            for i in range(5):
                ...
                row_label = QLabel(section_labels[i], self)
                ...
                row_label.setStyleSheet(f"""
                                        ...
                                        font-weight: bold;
                                        """)
                self.layout.addWidget(row_label, row_index, 2, 1, 2)  # Spanning 2 columns

                # Third column (table content, spans remaining 8 columns)
                table_cell = QLabel("", self)
                table_cell.setStyleSheet(f"""
                                         ...
                                         background-color: white;
                                         """)
                self.layout.addWidget(table_cell, row_index, 4, 1, 8)  # Spanning 8 columns

                row_index += 1  # Move to the next row

        # **Enclose the entire table with a thick border**
